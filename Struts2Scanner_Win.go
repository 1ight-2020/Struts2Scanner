...
func vulnerabilityChecking(url, req, poc string)  {
	...
	case strings.Contains(req,"Active Internet connections") == true:
		ColorPrint("[+]Linux目标存在"+poc+"漏洞", FontColor.red)
		addHistory(url+" find "+poc+" successfully\n")

	case strings.Contains(req,"Active Connections") == true || strings.Contains(req,"活动连接") == true:
		ColorPrint("[+]Windows目标存在"+poc+"漏洞", FontColor.red)
		addHistory(url+" find "+poc+" successfully\n")

	case strings.Contains(req,"LISTEN") == true:
		ColorPrint("[+]目标存在"+poc+"漏洞", FontColor.red)
		addHistory(url+" find "+poc+" successfully\n")

	default:
		ColorPrint("[-]目标不存在"+poc+"漏洞", FontColor.green)
	}
}
...
func getVulnerabilityScanner0452(url string) error {
	...
	if strings.Contains(reqs.Header.Get("testvuln"),"1522756") == true {
		ColorPrint("[+]目标存在struts2-045-2漏洞", FontColor.red)
		addHistory(url+" find struts2-045-2 successfully\n")
	}else {
		ColorPrint("[-]目标不存在struts2-045-2漏洞", FontColor.green)
	}
	return nil
}
...
func postVulnerabilityScanner020(url string, header map[string]string) error {
	...
	if reqs1.StatusCode == 200 && reqs2.StatusCode == 404 {
		ColorPrint("[+]Windows目标存在struts2-020漏洞（暂无POC）", FontColor.red)
		addHistory(url+" find struts2-020 successfully\n")
	}else {
		ColorPrint("[-]目标不存在struts2-020漏洞：", FontColor.green)
	}
	return nil
}
...
func getVulnerabilityScanner017(url string, header map[string]string) error {
	...
	if reqs.StatusCode == 302 {
		ColorPrint("[+]struts2-017检测成功（暂无可用POC）：", FontColor.red)
		addHistory(url+" find struts2-017 successfully\n")
	}else {
		ColorPrint("[-]目标不存在struts2-017漏洞", FontColor.green)
	}
	return nil
}
...
func postVulnerabilityScanner052(url string, header map[string]string) error {
	...
	if reqs.StatusCode == 500 &&  strings.Contains(string(body), "java.security.Provider$Service") == true{
		ColorPrint("[+]struts2-052检测成功（暂无可用POC，可参考MSF的struts2_rest_xstream模块）：", FontColor.red)
		addHistory(url+" find struts2-052 successfully\n")
	} else {
		ColorPrint("[-]目标不存在struts2-052漏洞", FontColor.green)
	}
	return nil
}
...
func vulnerabilityScanner(url string)  {
	...
	if err != nil {
		ColorPrint("[-]struts2-005检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-008-1检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-008-2检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-009检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-013检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {		
		ColorPrint("[-]struts2-016检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-017检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}

	req, err = postVulnerabilityScanner(poc["ST2_019"], url, header1)
	if err != nil {
		ColorPrint("[-]struts2-019检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-devmode检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-032检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-033检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-037检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-045检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-045-2检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}

	req, err = postVulnerabilityScanner046(url,header1)
	if err != nil {
		ColorPrint("[-]struts2-046检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-048检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
	...
	if err != nil {
		ColorPrint("[-]struts2-020检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}

	err = postVulnerabilityScanner052(url, header3)
	if err != nil {
		ColorPrint("[-]struts2-052检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}

	err = getVulnerabilityScanner053(url, header1)
	if err != nil {
		ColorPrint("[-]struts2-053检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}

	err = getVulnerabilityScanner057(url, header1)
	if err != nil {
		ColorPrint("[-]struts2-057-2检测失败：", FontColor.green)
		fmt.Printf("%v",err)
	}
}
...
func commandExecution(url, poc, cmd string)  {
	...
	case poc == "struts2-005":
		...
		if err != nil {
			ColorPrint("[-]"+cmd+"命令执行失败", FontColor.green)
			fmt.Printf("%v",err)
			return
		}
		fmt.Println(req)
	...
	case poc == "struts2-057":
		...
		if err != nil {
			ColorPrint("[-]struts2-057-1"+cmd+"命令执行失败，正在尝试struts2-057-2", FontColor.green)
			fmt.Printf("%v",err)
			reqs, errs := getShellCommandExecution057(url, cmd, "struts2-057-2", header1)
			if errs != nil {
				ColorPrint("[-]struts2-057-2"+cmd+"命令执行失败", FontColor.green)
				fmt.Printf("%v",err)
			}
			fmt.Println(reqs)
			return
	...
	default:
		ColorPrint("[-]请输入正确的漏洞名字" + "例如：Struts2Scanner -u " + url + "-n struts2-005", FontColor.green)
		os.Exit(0)
	}
}
func menu()  {
	ColorPrint(" ____  _              _           ____      ____                                  ", FontColor.light_purple)
	ColorPrint("/ ___|| |_ _ __ _   _| |_ ___    |___ \\    / ___|  ___ __ _ _ __  _ __   ___ _ __ ", FontColor.light_purple)
	ColorPrint("\\___ \\| __| '__| | | | __/ __|     __) |   \\___ \\ / __/ _` | '_ \\| '_ \\ / _ \\ '__|", FontColor.light_purple)
	ColorPrint(" ___) | |_| |  | |_| | |_\\__ \\    / __/     ___) | (_| (_| | | | | | | |  __/ |   ", FontColor.light_purple)
	ColorPrint("|____/ \\__|_|   \\__,_|\\__|___/___|_____|___|____/ \\___\\__,_|_| |_|_| |_|\\___|_|   ", FontColor.light_purple)
	ColorPrint( "                            |_____|   |_____|                                     ", FontColor.light_purple)
}
...
func main()  {
	...
	for {
		switch {
		case URL == "" && NAME == "" && FILE == "":
			ColorPrint( "请输入完整信息，例如：Struts2Scanner -u http://127.0.0.1/struts2-showcase/index.action -n struts2-005 或者 Struts2Scanner -f ./test.txt", FontColor.red)
			os.Exit(0)
		...
		case FILE != "":
			...
			if err != nil {
				ColorPrint( "请输入正确信息", FontColor.red)
				fmt.Println(err)
				os.Exit(0)
			}
			defer fi.Close()
			...
			for {
				...
				ColorPrint( string(url), FontColor.light_purple)
				if eof == io.EOF {
					break
				}
		...
		default:
				ColorPrint( "请输入完整信息，例如：Struts2Scanner -u http://127.0.0.1/struts2-showcase/index.action -n struts2-005", FontColor.red)
			os.Exit(0)
		}
	}
}